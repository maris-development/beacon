...
We are excited to announce the launch of a new **beacon node** dedicated to the [World Ocean Database (WOD)](https://www.ncei.noaa.gov/products/world-ocean-database) from NOAA, now made **publicly accessible for everyone**. This initiative is part of our ongoing commitment to open science and democratizing access to the world’s largest collection of ocean profile data.
...
Traditionally, accessing and analyzing such a vast trove of NetCDF files has been a daunting task. Downloading, indexing, and subsetting millions of files is slow, storage-intensive, and often out of reach for most researchers and data enthusiasts.

## Our Solution: Beacon Binary Format

To solve this, we leveraged the **beacon-binary-format**—a high-performance, open-source format designed to store and index millions of NetCDF datasets efficiently. Instead of handling millions of small files, we convert and store all WOD NetCDFs into just a few beacon-binary-format files. This approach:

- **Enables instant subsetting**: Query and extract just the data you need, on the fly, from the entire WOD collection.
...
Our beacon node is now live and open for public use. You can:

- **Run Jupyter notebooks** directly against the node
- **Query and subset** from 20+ million ocean profiles in real time
- **Integrate with your own tools** using our open APIs

Whether you’re a researcher, educator, or ocean enthusiast, this node puts the world’s ocean data at your fingertips—no special infrastructure required.
...
This is our contribution to a better world for open science. We invite you to explore, experiment, and help us push the boundaries of what’s possible with open ocean data.
