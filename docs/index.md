...
hero:
  ...
  tagline: "Query NetCDF, Zarr, Parquet, GeoTIFF and more in place, on local disk or S3. Embed the engine in Python with BeaconDB. Serve it to your team with Beacon Data Lake."
  image:
    src: /hero.png
    alt: Beacon
  actions:
...
features:
  - title: Fast analytical queries
    ...
    details: Beacon uses Rust, Apache Arrow and DataFusion. Filter and projection pushdown reads only the bytes that a query needs.
  ...
  - title: Works with existing formats
    ...
    details: Query NetCDF, Zarr, Parquet, GeoParquet, CSV, ODV, HDF5, Arrow, GeoTIFF, Delta and BBF files in place. Beacon does not convert your data first.
  ...
  - title: One SQL, everywhere
    ...
    details: Both options use the same SQL dialect and the same formats. Create external tables, managed tables, views and secrets. Use ATTACH to query a remote Beacon.
  ...
  - title: Open source and self-hosted
    ...
    details: Beacon uses the AGPL-3.0 license. Run Beacon on your laptop, your servers or your own cloud. You keep full control of your data.
