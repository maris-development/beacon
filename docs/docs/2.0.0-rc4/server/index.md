...
| | What it covers |
|---|---|
| **[Datasets & formats](/docs/2.0.0-rc4/server/datasets)** | Which files Beacon recognizes, and what each format supports |
| **[Tables & views](/docs/2.0.0-rc4/data-sources/)** | Giving files names: external tables, views, materialized views, crawlers, managed tables |
| **[Other sources](/docs/2.0.0-rc4/data-sources/object-storage)** | Data the node does not hold: a bucket, a Postgres database, another Beacon node |

Before this, [deploy the node](/docs/2.0.0-rc4/getting-started) and
[configure it](/docs/2.0.0-rc4/server/configuration). After it,
[decide who may read what](/docs/2.0.0-rc4/security/access-control) and
[point clients at it](/docs/2.0.0-rc4/connect/python).
...
- **External tables**: a registered name over one or more files. Give a folder or a glob pattern.
  ...
  [External Tables](/docs/2.0.0-rc4/data-sources/external-tables).
- **Managed tables**: tables that Beacon owns. You change them with `INSERT`, `UPDATE` and
  ...
  [Managed Tables](/docs/2.0.0-rc4/sql/managed-tables).
- **Views**: a saved query that behaves like a table. See
  [Views](/docs/2.0.0-rc4/server/view).
...
4. **Serve it to AI agents**, optional. Enable the `mcp` extension of a table. The agents then query
   the catalog over MCP. See [MCP Server](/docs/2.0.0-rc4/mcp).
-->

For the full detail, see the [SQL query docs](/docs/2.0.0-rc4/api/querying/sql) and the
[JSON query docs](/docs/2.0.0-rc4/api/querying/json).
...
GitHub Copilot. It uses its built-in [MCP Server](/docs/2.0.0-rc4/mcp). The catalog and the access
...
You enable each table with its `mcp` [extension](/docs/2.0.0-rc4/server/extensions). An agent
...
control the access. See [Access control](/docs/2.0.0-rc4/security/access-control).

The full [MCP Server guide](/docs/2.0.0-rc4/mcp) shows how to connect a client, how to restrict
authentication and how to serve a large result. An agent runs ordinary Beacon SQL. The
[SQL Guide](/docs/2.0.0-rc4/sql/) gives the full dialect.
