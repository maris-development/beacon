...
- Beacon supports Hive-style directory partitions. Use `PARTITIONED BY` on an [external table](/docs/2.0.0-rc4/data-sources/external-tables).
...
`STORED AS PARQUET` reads no key. Beacon ignores an `OPTIONS` clause on this format. See
