...
[Object Storage](/docs/2.0.0-rc4/data-sources/object-storage).

## Formats

| Format | Function | `STORED AS` | Recognized files |
| --- | --- | --- | --- |
| [Parquet](/docs/2.0.0-rc4/formats/parquet) | `read_parquet` | `PARQUET` | `.parquet` |
| [GeoParquet](/docs/2.0.0-rc4/formats/geoparquet) | `read_geoparquet` | `GEOPARQUET` | `.geoparquet` |
| [CSV / TSV](/docs/2.0.0-rc4/formats/csv) | `read_csv` | `CSV` | `.csv`, `.tsv` |
| [Arrow IPC](/docs/2.0.0-rc4/formats/arrow) | `read_arrow` | `ARROW` | `.arrow`, `.feather` |
| [NetCDF](/docs/2.0.0-rc4/formats/netcdf) | `read_netcdf` | `NC` | `.nc` |
| [HDF5](/docs/2.0.0-rc4/formats/hdf5) | `read_hdf5` | `HDF5`, `H5` | `.h5`, `.hdf5` |
| [Zarr](/docs/2.0.0-rc4/formats/zarr) | `read_zarr` | `ZARR` | `zarr.json` marker |
| [Atlas](/docs/2.0.0-rc4/formats/atlas) | `read_atlas` | `ATLAS` | `atlas.json` marker |
| [GeoTIFF / COG](/docs/2.0.0-rc4/formats/geotiff) | `read_tiff` | `TIFF` | `.tif`, `.tiff` |
| [BBF](/docs/2.0.0-rc4/formats/bbf) | `read_bbf` | `BBF` | `.bbf` |
| [Delta Lake](/docs/2.0.0-rc4/formats/delta-lake) | `read_delta` | `DELTA` | `_delta_log/` directory |
| [Apache Iceberg](/docs/2.0.0-rc4/formats/iceberg) | `read_iceberg` | `ICEBERG` | `metadata/` directory |
| [Icechunk](/docs/2.0.0-rc4/formats/icechunk) | `read_icechunk` | `ICECHUNK` | repository directory |
| [ODV ASCII](/docs/2.0.0-rc4/formats/odv) | `read_odv_ascii` | not supported | `.txt` |
...
| Icechunk | Full, see [Icechunk](/docs/2.0.0-rc4/formats/icechunk) | Projection + dimension selection, chunk pruning | No | `read_icechunk_schema` | Yes |
...
- **Pushdown** — how much of a query reaches storage instead of running after the read. *Predicate*
  ...
  [Atlas](/docs/2.0.0-rc4/formats/atlas) is the strongest: its collection statistics drop whole
...
- **Query output** — whether a query result can be written back in that format, with `COPY TO` or
  ...
  **Delta Lake** external tables accept `INSERT INTO`, and
  [managed tables](/docs/2.0.0-rc4/sql/managed-tables) accept the full `INSERT` / `UPDATE` /
...
- **Schema function** — the `_schema` counterpart that returns columns and types without a scan.
  See [Inspect a schema](/docs/2.0.0-rc4/formats/inspect-a-schema).
- **Array format** — whether the file holds N-dimensional arrays, which Beacon flattens into rows.
  See [Arrays to tables](/docs/2.0.0-rc4/arrays-to-tables).
...
[`SUMMARIZE`](/docs/2.0.0-rc4/sql/summarize) also gives value ranges, distinct counts and
...
[Inspect a schema](/docs/2.0.0-rc4/formats/inspect-a-schema) compares all four ways and says what
each one costs.
...
Array formats such as [Zarr](/docs/2.0.0-rc4/formats/zarr) and
[Atlas](/docs/2.0.0-rc4/formats/atlas) use a marker file. Point at `zarr.json`
or `atlas.json`, not at the chunk files.

Some files share a schema but have different columns. Combine those files with
[`UNION BY NAME`](/docs/2.0.0-rc4/sql/union-by-name).

## Give files a table name

A `read_*` call fits an ad-hoc query. When many queries share one source, register it once as an
[external table](/docs/2.0.0-rc4/data-sources/external-tables). Then query it by name:
...
- [Table Functions](/docs/2.0.0-rc4/sql/table-functions): every reader signature in one place.
- [Data Sources](/docs/2.0.0-rc4/data-sources/): the full read model.
- [Object Storage](/docs/2.0.0-rc4/data-sources/object-storage): running against a bucket.
