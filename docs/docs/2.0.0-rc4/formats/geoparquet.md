...
[`read_geoparquet()`](/docs/2.0.0-rc4/sql/table-functions#read-geoparquet) table function:
...
Use the [spatial functions](/docs/2.0.0-rc4/sql/spatial-functions). Those
...
geometry column on output. See [output formats](/docs/2.0.0-rc4/api/querying/).
:::
...
[GeoParquet in Supported Formats](/docs/2.0.0-rc4/server/datasets#supported-formats) for the
details.

See [Create External Tables](/docs/2.0.0-rc4/data-sources/external-tables) for the full DDL. See [Data Sources](/docs/2.0.0-rc4/data-sources/) for the
full read model.

### `OPTIONS`

`STORED AS GEOPARQUET` reads no key. Beacon ignores an `OPTIONS` clause on this format. See
[`OPTIONS`](/docs/2.0.0-rc4/sql/create-external-table#options) for the formats that do read one.
