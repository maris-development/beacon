...
Then use a [managed table](/docs/2.0.0-rc4/sql/managed-tables).
:::

:::tip Register many datasets at once
A [crawler](/docs/2.0.0-rc4/server/crawlers) finds and registers many datasets under a prefix. It
...
[`CREATE EXTERNAL TABLE`](/docs/2.0.0-rc4/sql/create-external-table) reference gives the full
statement grammar and every clause: `OR REPLACE`, `IF NOT EXISTS`, `PARTITIONED BY`, `OPTIONS` and
`DROP TABLE`.
[Keys of each format](/docs/2.0.0-rc4/sql/create-external-table#keys-of-each-format) lists the
...
[GeoParquet](/docs/2.0.0-rc4/formats/geoparquet) for the details.
...
See [Atlas](/docs/2.0.0-rc4/formats/atlas) for the format details. That page
...
the [`read_odv_ascii()`](/docs/2.0.0-rc4/sql/table-functions#read-odv-ascii) table function.
...
[Delta Lake](/docs/2.0.0-rc4/formats/delta-lake) table. The `LOCATION` points
...
new Delta version. See [Delta Lake](/docs/2.0.0-rc4/formats/delta-lake) for the
full reference.

### Apache Iceberg

`STORED AS ICEBERG` registers an [Apache Iceberg](/docs/2.0.0-rc4/formats/iceberg)
...
[Apache Iceberg](/docs/2.0.0-rc4/formats/iceberg) for the full reference.

### Icechunk

`STORED AS ICECHUNK` adds an [Icechunk](/docs/2.0.0-rc4/formats/icechunk) repository to the
...
The table is read only. See [Icechunk](/docs/2.0.0-rc4/formats/icechunk) for the full reference.
...
[`PARTITIONED BY`](/docs/2.0.0-rc4/sql/create-external-table#partitioned-by) for the syntax.
...
[Remote Tables (Federation)](/docs/2.0.0-rc4/data-sources/remote-tables) for the full
reference.

## Views

A view holds a persistent SQL query over any external table or table function. See the
[Views](/docs/2.0.0-rc4/server/view) page for the full reference. It also covers
...
[`DROP TABLE`](/docs/2.0.0-rc4/sql/create-external-table#drop-table).
