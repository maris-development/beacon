...
For a table that Beacon owns and writes, see [`CREATE TABLE`](/docs/2.0.0-rc4/sql/managed-tables).
...
| ----------- | ---------- | ----------- |
| `PARQUET` | `.parquet` | [Parquet](/docs/2.0.0-rc4/formats/parquet) |
| `GEOPARQUET` | `.geoparquet` | [GeoParquet](/docs/2.0.0-rc4/formats/geoparquet) |
| `NC` | `.nc` | [NetCDF](/docs/2.0.0-rc4/formats/netcdf) |
| `HDF5`, `H5` | `.h5`, `.hdf5` | [HDF5](/docs/2.0.0-rc4/formats/hdf5) |
| `ZARR` | Zarr v3 (`zarr.json`) | [Zarr](/docs/2.0.0-rc4/formats/zarr) |
| `ATLAS` | Atlas array store (`atlas.json`) | [Atlas](/docs/2.0.0-rc4/formats/atlas) |
| `CSV` | `.csv`, `.tsv` | [CSV](/docs/2.0.0-rc4/formats/csv) |
| `ARROW` | Arrow IPC (`.arrow`, `.feather`) | [Arrow IPC](/docs/2.0.0-rc4/formats/arrow) |
| `TIFF` | GeoTIFF / Cloud-Optimized GeoTIFF | [GeoTIFF](/docs/2.0.0-rc4/formats/geotiff) |
| `BBF` | Beacon Binary Format | [BBF](/docs/2.0.0-rc4/formats/bbf) |
| `DELTA` | Delta Lake table directory (`_delta_log/`) | [Delta Lake](/docs/2.0.0-rc4/formats/delta-lake) |
| `ICEBERG` | Apache Iceberg table directory (`metadata/`) | [Apache Iceberg](/docs/2.0.0-rc4/formats/iceberg) |
| `ICECHUNK` | Icechunk repository directory | [Icechunk](/docs/2.0.0-rc4/formats/icechunk) |
| `POSTGRES` | External PostgreSQL table (federated) | [SQL Databases](/docs/2.0.0-rc4/data-sources/sql-databases) |
| `MYSQL` | External MySQL table (federated) | [SQL Databases](/docs/2.0.0-rc4/data-sources/sql-databases) |
| `REMOTE` | A table on another Beacon server | [Remote Tables](/docs/2.0.0-rc4/data-sources/remote-tables) |

`DELTA` points at an existing
[Delta Lake](/docs/2.0.0-rc4/formats/delta-lake) table directory. It also
supports time travel and `INSERT INTO`. `ICEBERG` points to an [Apache Iceberg](/docs/2.0.0-rc4/formats/iceberg) table
...
[Remote Tables](/docs/2.0.0-rc4/data-sources/remote-tables). `POSTGRES` and `MYSQL`
federate a table in an external SQL database. See
[SQL Databases](/docs/2.0.0-rc4/data-sources/sql-databases). Their `LOCATION` is the remote
...
[GeoParquet in File Formats](/docs/2.0.0-rc4/formats/geoparquet) for the read
...
[File formats](/docs/2.0.0-rc4/server/configuration#file-formats) settings hold those defaults. A
...
| ----------- | ---- | ------- |
| `NC` | `read_dimensions`, `use_rust_reader`, `enable_statistics` | [NetCDF](/docs/2.0.0-rc4/formats/netcdf#options) |
| `HDF5`, `H5` | `read_dimensions`, `use_rust_reader`, `enable_statistics`, `unify_phony_dimensions`, `convention` | [HDF5](/docs/2.0.0-rc4/formats/hdf5#options) |
| `ZARR` | `read_dimensions`, `enable_statistics` | [Zarr](/docs/2.0.0-rc4/formats/zarr#options) |
| `CSV` | `delimiter`, `infer_records` | [CSV](/docs/2.0.0-rc4/formats/csv#options) |
| `BBF` | `split_streams_slice` | [BBF](/docs/2.0.0-rc4/formats/bbf#options) |
| `DELTA` | `version`, `timestamp` | [Delta Lake](/docs/2.0.0-rc4/formats/delta-lake#options) |
| `ICEBERG` | `snapshot_id` | [Apache Iceberg](/docs/2.0.0-rc4/formats/iceberg#options) |
| `ICECHUNK` | `branch`, `tag`, `snapshot`, `read_dimensions` | [Icechunk](/docs/2.0.0-rc4/formats/icechunk#options) |
| `POSTGRES`, `MYSQL` | `host`, `port`, `user`, `password`, `database`, `sslmode` | [SQL Databases](/docs/2.0.0-rc4/data-sources/sql-databases#options) |
| `REMOTE` | `tls` | [Remote Tables](/docs/2.0.0-rc4/data-sources/remote-tables#options) |
...
The [External Tables](/docs/2.0.0-rc4/data-sources/external-tables) setup guide gives an
