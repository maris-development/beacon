...
[remote table](/docs/2.0.0-rc2/beacondb/data-sources/remote-tables). Both use Arrow Flight SQL
...
[remote table](/docs/2.0.0-rc2/beacondb/data-sources/remote-tables) when you need one table under a
fixed local name.
...
- **Credentials**: give exactly one of three options. Use `'username'` and `'password'` for HTTP
  ...
  [`TYPE BEACON` secret](/docs/2.0.0-rc2/beacondb/sql/secrets). Omit all three for anonymous access.
...
[Remote Tables](/docs/2.0.0-rc2/beacondb/data-sources/remote-tables#how-pushdown-works) for the
...
The embedded engine gives the same function as `con.attach(name, url, …)`. See
