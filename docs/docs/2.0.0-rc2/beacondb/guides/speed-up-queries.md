---
description: Diagnose a slow BeaconDB query with EXPLAIN. Then fix it with pushdown, a better file layout, Atlas, a materialized view or a managed table.
---
...
- **Use a cloud-optimized format** for data on object storage. See
  [Query Data on S3](/docs/2.0.0-rc2/beacondb/guides/query-s3).

## 5. Merge large array collections with Atlas

Do you query a large NetCDF or Zarr collection often? Then convert it to
[Atlas](/docs/2.0.0-rc2/beacondb/data-sources/formats/atlas). This gives the largest gain. Atlas
...
A [materialized view](/docs/2.0.0-rc2/beacondb/sql/create-materialized-view) caches the result. You
...
Use a [managed table](/docs/2.0.0-rc2/beacondb/data-sources/internal-format) for a working subset
...
See [Export Query Results](/docs/2.0.0-rc2/beacondb/guides/export-results).

## Server-side tuning

Beacon Data Lake adds more settings. They cover concurrency, memory, disk spill, reader caches and
object store listings.
[Performance Tuning](/docs/2.0.0-rc2/data-lake/performance-tuning) documents them.
