...
[`EXPLAIN`](/docs/2.0.0-rc2/sql/select).
...
[output formats](/docs/2.0.0-rc2/api/querying/#output-formats).

For a streaming read in Python instead of a file, connect over Arrow Flight SQL and pull record
batches. See [Python ADBC](/docs/2.0.0-rc2/connect/python-adbc).
...
This creates a [managed table](/docs/2.0.0-rc2/sql/managed-tables) inside `beacon.db`. Does the
...
See [output formats](/docs/2.0.0-rc2/api/querying/#output-formats) for the full option set of each
one.
