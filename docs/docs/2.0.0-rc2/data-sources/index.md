...
[its own internal format](/docs/2.0.0-rc2/internals/storage).

| Source | Read with | Chapter |
| --- | --- | --- |
| Files on local disk | `read_*()` functions or external tables | [File Formats](/docs/2.0.0-rc2/formats/) |
| Datasets on an S3 bucket | the same relative paths; the store is set at startup | [Object Storage](/docs/2.0.0-rc2/data-sources/object-storage) |
| A named set of files | `CREATE EXTERNAL TABLE` | [External Tables](/docs/2.0.0-rc2/data-sources/external-tables) |
| Postgres, MySQL, ODBC | federated external tables | [SQL Databases](/docs/2.0.0-rc2/data-sources/sql-databases) |
| One table on another Beacon | `STORED AS REMOTE` | [Remote Tables](/docs/2.0.0-rc2/data-sources/remote-tables) |
| A whole remote Beacon catalog | `ATTACH` | [ATTACH](/docs/2.0.0-rc2/data-sources/attach) |
| Data that Beacon owns and can change | managed tables in `beacon.db` | [Storage internals](/docs/2.0.0-rc2/internals/storage) |
...
such as [Zarr](/docs/2.0.0-rc2/formats/zarr) and
...
the limitations. See [File Formats](/docs/2.0.0-rc2/formats/) for the full
list. The [table functions reference](/docs/2.0.0-rc2/sql/table-functions) holds every
...
- An **[external table](/docs/2.0.0-rc2/data-sources/external-tables)** puts a stable name
...
[secrets](/docs/2.0.0-rc2/sql/secrets). Secrets replace scattered environment variables.
