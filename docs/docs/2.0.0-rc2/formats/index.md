...
| --- | --- | --- | --- |
| [Parquet](/docs/2.0.0-rc2/formats/parquet) | `read_parquet` | `PARQUET` | `.parquet` |
| [GeoParquet](/docs/2.0.0-rc2/formats/geoparquet) | `read_geoparquet` | `GEOPARQUET` | `.geoparquet` |
| [CSV / TSV](/docs/2.0.0-rc2/formats/csv) | `read_csv` | `CSV` | `.csv`, `.tsv` |
| [Arrow IPC](/docs/2.0.0-rc2/formats/arrow) | `read_arrow` | `ARROW` | `.arrow`, `.feather` |
| [NetCDF](/docs/2.0.0-rc2/formats/netcdf) | `read_netcdf` | `NC` | `.nc` |
| [HDF5](/docs/2.0.0-rc2/formats/hdf5) | `read_hdf5` | `HDF5`, `H5` | `.h5`, `.hdf5` |
| [Zarr](/docs/2.0.0-rc2/formats/zarr) | `read_zarr` | `ZARR` | `zarr.json` marker |
| [Atlas](/docs/2.0.0-rc2/formats/atlas) | `read_atlas` | `ATLAS` | `atlas.json` marker |
...
| [Apache Iceberg](/docs/2.0.0-rc2/formats/iceberg) | `read_iceberg` | `ICEBERG` | `metadata/` directory |
...
and **ODV ASCII** are the exception. Point a function at them. For Delta and Iceberg, you can also
...
| NetCDF | **Anonymous only** | Projection + dimension selection | Yes | `read_netcdf_schema` | Yes |
| HDF5 | **Anonymous only** | Projection + dimension selection | Yes | `read_hdf5_schema` | Yes |
...
- **On an S3 datasets store** — how the reader behaves when the server's datasets store is a
  ...
  reads**: netCDF-c opens a file by URL and never sees the credential chain. Every other reader
  goes through the object store and authenticates normally. This is a property of the server's
  store, not of the query — paths in SQL are relative either way.
...
- **Query output** — whether a query result can be written back in that format, with `COPY TO` or
  ...
  **Delta Lake** external tables accept `INSERT INTO`, and
  [managed tables](/docs/2.0.0-rc2/sql/managed-tables) accept the full `INSERT` / `UPDATE` /
...
- **Schema function** — the `_schema` counterpart that returns columns and types without a scan.
  See [Inspect a schema](/docs/2.0.0-rc2/formats/inspect-a-schema).
- **Array format** — whether the file holds N-dimensional arrays, which Beacon flattens into rows.
  See [Arrays to tables](/docs/2.0.0-rc2/arrays-to-tables).
...
[`SUMMARIZE`](/docs/2.0.0-rc2/sql/summarize) also gives value ranges, distinct counts and
...
[Inspect a schema](/docs/2.0.0-rc2/formats/inspect-a-schema) compares all four ways and says what
each one costs.
...
Array formats such as [Zarr](/docs/2.0.0-rc2/formats/zarr) and
[Atlas](/docs/2.0.0-rc2/formats/atlas) use a marker file. Point at `zarr.json`
or `atlas.json`, not at the chunk files.

Some files share a schema but have different columns. Combine those files with
[`UNION BY NAME`](/docs/2.0.0-rc2/sql/union-by-name).

## Give files a table name

A `read_*` call fits an ad-hoc query. When many queries share one source, register it once as an
[external table](/docs/2.0.0-rc2/data-sources/external-tables). Then query it by name:
...
- [Table Functions](/docs/2.0.0-rc2/sql/table-functions): every reader signature in one place.
- [Data Sources](/docs/2.0.0-rc2/data-sources/): the full read model.
- [Object Storage](/docs/2.0.0-rc2/data-sources/object-storage): running against a bucket.
