...
exploration. See the [`read_delta`](/docs/2.0.0-rc2/sql/table-functions#read-delta) table
function.
...
- **`INSERT` appends only.** Beacon does not expose `UPDATE`, `DELETE` or `MERGE` on a Delta table.
  Use a [managed table](/docs/2.0.0-rc2/sql/managed-tables) to change rows.
