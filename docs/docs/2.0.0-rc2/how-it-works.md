...
[external table](/docs/2.0.0-rc2/data-sources/external-tables) and query them by name.
...
- **Predicate pushdown**: Beacon turns filters into file, row group and chunk pruning. A time range
  filter on a [Zarr](/docs/2.0.0-rc2/formats/zarr) store fetches only the
  chunks in that range. On [Atlas](/docs/2.0.0-rc2/formats/atlas), Beacon uses
...
- **Federated pushdown**: Beacon sends filters, projections, limits and whole aggregates to
  [SQL databases](/docs/2.0.0-rc2/data-sources/sql-databases) and to
  [remote Beacons](/docs/2.0.0-rc2/data-sources/remote-tables). Only the reduced result
  travels back.
...
[Arrays to tables](/docs/2.0.0-rc2/arrays-to-tables) specifies the mapping: the row count, the
broadcast rule and the `dimensions` argument. [File Formats](/docs/2.0.0-rc2/formats/) documents the
behaviour of each format.

## Storage: what Beacon owns

Most data stays where it is. Beacon *owns* the content of one
[`beacon.db` file](/docs/2.0.0-rc2/internals/storage). That file holds the
...
See [Access Control](/docs/2.0.0-rc2/security/access-control) for the grant model, and
[Getting Started](/docs/2.0.0-rc2/getting-started) to run a server.
