...
[reader function](/docs/2.0.0-rc2/formats/) or an
...
| --- | --- | --- | --- | --- |
| [Parquet](/docs/2.0.0-rc2/formats/parquet) | `.parquet` | `PARQUET` | `read_parquet` | yes |
| [GeoParquet](/docs/2.0.0-rc2/formats/geoparquet) | `.geoparquet` | `GEOPARQUET` | `read_geoparquet` | yes |
| [CSV / TSV](/docs/2.0.0-rc2/formats/csv) | `.csv`, `.tsv` | `CSV` | `read_csv` | yes |
| [Arrow IPC](/docs/2.0.0-rc2/formats/arrow) | `.arrow`, `.feather` | `ARROW` | `read_arrow` | yes (`ipc`) |
| [NetCDF](/docs/2.0.0-rc2/formats/netcdf) | `.nc` | `NC` | `read_netcdf` | yes (+ ND-NetCDF) |
| [Zarr](/docs/2.0.0-rc2/formats/zarr) | `zarr.json` marker | `ZARR` | `read_zarr` | no |
| [Atlas](/docs/2.0.0-rc2/formats/atlas) | `atlas.json` marker | `ATLAS` | `read_atlas` | no |
...
limitations and the tuning. See [External Files](/docs/2.0.0-rc2/formats/).
:::
...
- **Object storage**: an S3, GCS or Azure prefix. See
  [Object Storage](/docs/2.0.0-rc2/data-sources/object-storage) for the credentials and the
  setup.

Beacon reads the files in place. Beacon never copies or converts them. A new file is queryable at
once. Use a [crawler](/docs/2.0.0-rc2/server/crawlers) to register many datasets under a prefix
as named tables in one step. A crawler also handles a partitioned layout.

## Next

- **[External Files](/docs/2.0.0-rc2/formats/)**: the read reference for each format.
- **[Create External Tables](/docs/2.0.0-rc2/data-sources/external-tables)**: give a set of files a stable table name.
- **[Performance Tuning](/docs/2.0.0-rc2/server/performance-tuning)**: the layout and format choices that make a scan faster.
