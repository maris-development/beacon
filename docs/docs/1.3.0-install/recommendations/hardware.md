...
# Server Hardware Recommendations for Beacon
...
- **RAM**: At least 4 GB (may vary depending on the number of datasets to run parallel queries on)
- **Storage**: 
  - HDD with at least 50 GB free space (to store temporary files for processing & output)
...
- **Network**: 
  - 10 Gbps Ethernet for high-speed data transfer (especially important for cloud deployments)
...
- **Optimal**: 
  ...
  - Example: Samsung PM1735 NVMe SSD with RAID 5 on a 4 x 2 TB NVMe SSDs for high-speed data access and fault tolerance. (For local deployments)

### Network
- **Minimum**: 1 Gbps Ethernet
- **Recommended**: 
  - 10 Gbps Ethernet for high-speed data transfer and reduced latency (especially important for cloud deployments).
...
- **Operating System**: 
  - Compatible with server editions of major operating systems: Ubuntu Server 22.04 LTS (or newer), Windows Server 2022 (or newer), MacOS (Monterey or newer).
