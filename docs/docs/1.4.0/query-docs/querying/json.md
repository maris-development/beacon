...
It is also possible to query from datasets directly by using the `from` field. In that case, you need to provide the format and the path to the dataset(s) that you want to query. You can use glob patterns to query multiple datasets at once.
...
Here you select the output format and choose between Parquet, Arrow, CSV, netCDF and ODV
...
```json
{
    "format": {
        "geoparquet": {
            "longitude_column" : "{query_parameter_alias_longitude}",
            "latitude_column" : "{query_parameter_alias_latitude}"
        }
    }
}
```
