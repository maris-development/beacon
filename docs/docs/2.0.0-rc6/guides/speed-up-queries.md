...
- **Partition** the files Hive-style, for example `year=2024/month=01/`. Declare the partition
  ...
  [`CREATE EXTERNAL TABLE`](/docs/2.0.0-rc6/sql/create-external-table).
...
- **Use a cloud-optimized format** for data on object storage. See
  [Query Data on S3](/docs/2.0.0-rc6/guides/query-s3).

## 5. Merge large array collections with Atlas

Do you query a large NetCDF or Zarr collection often? Then convert it to
[Atlas](/docs/2.0.0-rc6/formats/atlas). This gives the largest gain. Atlas
...
A [materialized view](/docs/2.0.0-rc6/sql/create-materialized-view) caches the result. You
...
Use a [managed table](/docs/2.0.0-rc6/internals/storage) for a working subset
...
See [Export Query Results](/docs/2.0.0-rc6/guides/export-results).

## Server-side tuning

The server adds more settings. They cover concurrency, memory, disk spill, reader caches and
object store listings.
[Performance Tuning](/docs/2.0.0-rc6/server/performance-tuning) documents them.
