...
See [Excluded variables](/docs/2.0.0-rc6/arrays-to-tables#excluded-variables).
...
the list smaller. See [Arrays to tables](/docs/2.0.0-rc6/arrays-to-tables).
...
[Supported calendars](/docs/2.0.0-rc6/cf-decoding#supported-calendars).
...
See [CF decoding](/docs/2.0.0-rc6/cf-decoding).
...
See [`read_<format>_schema`](/docs/2.0.0-rc6/sql/table-functions-utility#read-format-schema).
...
datasets store is that bucket. See [NetCDF](/docs/2.0.0-rc6/formats/netcdf).
...
### A column has two types across the files

A collection can type one column as a number in one file and as a string in another. No type holds
both, so the table answers no query:

```
Incompatible types for field 'platform': Int32 in 'argo/2019.nc' vs Utf8 in 'argo/2020.nc'
```

The message names the column, both types and both files. Fix the files, or drop the column from the
table, or keep the type of the first file:
...
[Configuration](/docs/2.0.0-rc6/server/configuration#query-engine).

### New files do not appear

Beacon does not watch storage. It reads no filesystem events and no S3 events. Register a
[crawler](/docs/2.0.0-rc6/server/crawlers) with a schedule.
...
tree, use a [crawler](/docs/2.0.0-rc6/server/crawlers). The crawler does this work in advance.
...
See [Speed up slow queries](/docs/2.0.0-rc6/guides/speed-up-queries).
...
over Flight SQL. See [Export query results](/docs/2.0.0-rc6/guides/export-results).

A server-side query can spill to disk. The spill goes to the OS temp area. Put that on fast storage
with free space. See [Performance Tuning](/docs/2.0.0-rc6/server/performance-tuning).
...
See [Configuration](/docs/2.0.0-rc6/server/configuration).
...
[Secrets](/docs/2.0.0-rc6/sql/secrets).
...
For an authenticated connection, use [`ATTACH`](/docs/2.0.0-rc6/data-sources/attach) with a secret.
