...
does not fail. See [UNION BY NAME](/docs/2.0.0-rc6/sql/union-by-name).
...
- **A Beacon result is a table, not a `Dataset`.** The array shape is flattened into rows. See
  [Arrays to tables](/docs/2.0.0-rc6/arrays-to-tables) for the row count and the broadcast rule.
...
- **Non-standard calendars are unsupported.** `noleap` and `360_day` files are rejected. See
  [CF decoding](/docs/2.0.0-rc6/cf-decoding#supported-calendars).
...
- [Arrays to tables](/docs/2.0.0-rc6/arrays-to-tables): what a dimension does to your row count
- [CF decoding](/docs/2.0.0-rc6/cf-decoding): units, packing and fill values
- [Query a file collection](/docs/2.0.0-rc6/guides/query-a-collection): the full walkthrough
