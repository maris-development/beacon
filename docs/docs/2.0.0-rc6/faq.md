...
[Query the public node](/docs/2.0.0-rc6/quickstart#query-the-public-node).
...
[`read_*()` function](/docs/2.0.0-rc6/formats/) or an
[external table](/docs/2.0.0-rc6/data-sources/external-tables) at a path. Then query it.
...
[File Formats](/docs/2.0.0-rc6/formats/).

## Read files

### How do I see what columns and types a file has?

[`read_<format>_schema()`](/docs/2.0.0-rc6/sql/table-functions-utility#read-format-schema) returns the
...
Delta Lake and ODV, use a `LIMIT 0` query. [`SUMMARIZE`](/docs/2.0.0-rc6/sql/summarize) also
works.

### My files have different columns

The query fails or returns nulls. Use
[`UNION BY NAME`](/docs/2.0.0-rc6/sql/union-by-name) to combine the files by column name,
...
[`read_odv_ascii()`](/docs/2.0.0-rc6/formats/odv) or
[`read_delta()`](/docs/2.0.0-rc6/formats/delta-lake) or
[`read_iceberg()`](/docs/2.0.0-rc6/formats/iceberg) at the data. For Delta and Iceberg,
...
[view](/docs/2.0.0-rc6/server/view) to get a stable name.
...
[secret](/docs/2.0.0-rc6/sql/secrets):

```sql
CREATE SECRET my_s3 (TYPE S3, KEY_ID '…', SECRET '…', REGION 'eu-west-1', SCOPE 's3://my-bucket');
```

See [Query Data on S3](/docs/2.0.0-rc6/guides/query-s3).
...
collection to [Zarr](/docs/2.0.0-rc6/formats/zarr) or
[Atlas](/docs/2.0.0-rc6/formats/atlas). Both formats have full object storage
support.
...
collections into [Atlas](/docs/2.0.0-rc6/formats/atlas). See
[Speed Up Slow Queries](/docs/2.0.0-rc6/guides/speed-up-queries) for the full detail.
...
[what the scan skips](/docs/2.0.0-rc6/formats/geoparquet).
...
[Performance Tuning](/docs/2.0.0-rc6/server/performance-tuning).
...
[server](/docs/2.0.0-rc6/getting-started) and connect every client to it.
...
[Storage internals](/docs/2.0.0-rc6/internals/storage).
...
[materialized view](/docs/2.0.0-rc6/sql/create-materialized-view).
...
[Access Control](/docs/2.0.0-rc6/security/access-control).
...
an authenticated connection, use [`ATTACH`](/docs/2.0.0-rc6/data-sources/attach). It accepts
...
Use a [crawler](/docs/2.0.0-rc6/server/crawlers) with a schedule to find new files.
...
See [Python client](/docs/2.0.0-rc6/connect/python). For a terminal instead of a notebook, install
[`beacon-datalake-cli`](/docs/2.0.0-rc6/connect/cli).

### Can I connect a SQL tool instead?

Yes. The server speaks Arrow Flight SQL and JDBC. See
[DataGrip / JDBC](/docs/2.0.0-rc6/connect/datagrip) and
[Python ADBC](/docs/2.0.0-rc6/connect/python-adbc).

---

Do you still have a problem? Open an issue on
[GitHub](https://github.com/maris-development/beacon). Or read
[Concepts](/docs/2.0.0-rc6/concepts) for the full picture.
