...
attributes as a plain [Zarr](/docs/2.0.0-rc6/formats/zarr) store. It adds commits, branches, tags
and snapshots.
...
[Arrays to tables](/docs/2.0.0-rc6/arrays-to-tables) shows how Beacon changes an array into rows.
...
- An S3 datasets store reads over HTTP without a signature. The bucket must allow anonymous reads.
  [NetCDF](/docs/2.0.0-rc6/formats/netcdf) works the same way.
