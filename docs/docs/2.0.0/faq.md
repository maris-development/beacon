...
[Which should I use?](/docs/2.0.0/introduction#which-should-i-use).

### Do I need to import or convert my files first?

No. Beacon reads files in place. Point a
[`read_*()` function](/docs/2.0.0/beacondb/data-sources/formats/) or an
[external table](/docs/2.0.0/beacondb/data-sources/external-tables) at a path and query it. Nothing
is copied or rewritten.

### Which formats are supported?

Parquet, GeoParquet, CSV/TSV, Arrow IPC, NetCDF, Zarr, Atlas, GeoTIFF/COG, BBF, Delta Lake, and ODV
ASCII. Each has its own chapter in [File Formats](/docs/2.0.0/beacondb/data-sources/formats/).

## Reading files

### How do I see what columns and types a file has?

[`read_schema()`](/docs/2.0.0/beacondb/sql/table-functions-utility#read_schema) returns the schema
...
[`SUMMARIZE`](/docs/2.0.0/beacondb/sql/summarize) instead.
...
[`UNION BY NAME`](/docs/2.0.0/beacondb/sql/union-by-name). Missing columns become nulls instead of
misaligning.
...
[`read_odv_ascii()`](/docs/2.0.0/beacondb/data-sources/formats/odv) or
[`read_delta()`](/docs/2.0.0/beacondb/data-sources/formats/delta-lake) at them directly, or register
...
[view](/docs/2.0.0/data-lake/view) if you want a stable name.
...
[secret](/docs/2.0.0/beacondb/sql/secrets):

```sql
CREATE SECRET my_s3 (TYPE S3, KEY_ID '…', SECRET '…', REGION 'eu-west-1', SCOPE 's3://my-bucket');
```

See [Query Data on S3](/docs/2.0.0/beacondb/guides/query-s3).
...
[Zarr](/docs/2.0.0/beacondb/data-sources/formats/zarr) or
[Atlas](/docs/2.0.0/beacondb/data-sources/formats/atlas).
...
collections into [Atlas](/docs/2.0.0/beacondb/data-sources/formats/atlas). Full detail in
[Speed Up Slow Queries](/docs/2.0.0/beacondb/guides/speed-up-queries).
...
[Beacon Data Lake](/docs/2.0.0/getting-started) and connect to it instead.
...
[Internal Format](/docs/2.0.0/beacondb/data-sources/internal-format).
...
a [materialized view](/docs/2.0.0/beacondb/sql/create-materialized-view) is usually the better fit.
...
authenticated federation, use [`ATTACH`](/docs/2.0.0/beacondb/data-sources/attach), which accepts a
...
[SQLAlchemy](/docs/2.0.0/beacondb/python/sqlalchemy).

---

Still stuck? Open an issue on
[GitHub](https://github.com/maris-development/beacon), or check
[Concepts](/docs/2.0.0/concepts) for how the pieces fit together.
