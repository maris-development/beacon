...
That creates a [managed table](/docs/2.0.0/beacondb/data-sources/internal-format) inside `beacon.db`.
If the underlying data changes and you want the result refreshed on demand, use a
[materialized view](/docs/2.0.0/beacondb/sql/create-materialized-view) instead.
...
See [querying output formats](/docs/2.0.0/api/querying/#output-formats).
