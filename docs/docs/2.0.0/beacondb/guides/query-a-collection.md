...
[`SUMMARIZE`](/docs/2.0.0/beacondb/sql/summarize) returns one row per column with types, null counts,
...
See [UNION BY NAME](/docs/2.0.0/beacondb/sql/union-by-name).
...
See [External Tables](/docs/2.0.0/beacondb/data-sources/external-tables).

## 6. If it is slow

A repeated scan over thousands of NetCDF or Zarr files is the classic case for consolidating into an
[Atlas](/docs/2.0.0/beacondb/data-sources/formats/atlas) collection, which prunes whole datasets
