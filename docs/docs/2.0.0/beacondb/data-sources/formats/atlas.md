...
Query an Atlas store with the [`read_atlas()`](/docs/2.0.0/beacondb/sql/table-functions#read_atlas) table function, pointing at its `atlas.json` marker file, an exact path or a glob such as `**/atlas.json`. An optional second argument filters the arrays to those matching the listed dimensions.
...
For a stable, reusable table name, register the store as an [External Table](/docs/2.0.0/beacondb/data-sources/external-tables#atlas). Like Zarr, point the `LOCATION` at the `atlas.json` marker (or a glob over several markers):
...
:::tip
Heavy, repeated aggregations over an Atlas collection, or any other table, can be cached with a [materialized view](/docs/2.0.0/beacondb/sql/create-materialized-view) and recomputed with `REFRESH` when the underlying data changes.
:::
