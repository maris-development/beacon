...
For collections that are queried repeatedly, convert the Zarr stores into a single [Atlas](/docs/2.0.0/beacondb/data-sources/formats/atlas) collection, a statistics-aware array store that lets Beacon prune whole datasets before any chunk is read.
:::
