...
- **`mcp`**: expose the table to AI agents as a read-only [MCP](/docs/2.0.0-rc1/mcp) tool.
...
Enabling the `mcp` extension makes the table discoverable to AI agents and generates a dedicated per-table tool on Beacon's [MCP Server](/docs/2.0.0-rc1/mcp). The tool is **read-only** and constrained to the columns you expose.
...
These are nudges only. Actual result-size limits come from the built-in `run_sql` preview cap, see the [MCP Server guide](/docs/2.0.0-rc1/mcp) for the full tool set, connecting a client, and authenticating agents.
...
- [MCP Server](/docs/2.0.0-rc1/mcp), connect an agent, authenticate it, and handle large results.
-->
- [External Tables](/docs/2.0.0-rc1/beacondb/data-sources/external-tables), registering the tables you attach extensions to.
- [Access control](/docs/2.0.0-rc1/security/access-control), the identity and role grants that govern what agents can read.
