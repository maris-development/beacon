...
- If listings are slow, use a [crawler](/docs/2.0.0-rc1/data-lake/crawlers) so the catalog is
...
::: tip
For collections that are queried repeatedly, re-encode the Zarr stores into a single [Atlas](/docs/2.0.0-rc1/beacondb/data-sources/formats/atlas) collection. Atlas adds per-dataset statistics pruning on top of chunk pruning, dropping whole datasets before any chunk is read.
:::

## Atlas Tuning

[Atlas](/docs/2.0.0-rc1/beacondb/data-sources/formats/atlas) stores are opened through a `atlas.json` registry. To avoid re-opening the same store on every query, Beacon keeps a cache of opened Atlas readers.
