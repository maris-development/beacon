...
Open <http://localhost:5001/admin> and sign in with the admin username and password you set above (`admin` / `securepassword`). Beacon bundles an [admin web UI](/docs/2.0.0-rc1/connect/web-admin-ui) into the server and Docker image, nothing extra to deploy. From it you can:
...
If you used Compose, start it with `docker compose up -d`. Beacon is now running. Open the [admin UI](/docs/2.0.0-rc1/connect/web-admin-ui) at `http://localhost:5001/admin` to explore and query, or `http://localhost:5001/swagger` for the API docs. Any files placed in `./datasets` are immediately available for querying.

::: tip Two ways to connect
Beacon exposes two endpoints. The **HTTP API** on port `5001` serves SQL/JSON queries, the admin UI, and the OpenAPI docs. The **Arrow Flight SQL** server on port `32011` is a high-throughput, columnar protocol used by clients such as [JetBrains DataGrip](/docs/2.0.0-rc1/connect/jetbrains-datagrip) and the [Python ADBC driver](/docs/2.0.0-rc1/connect/python-adbc). Flight SQL uses bearer-token authentication and can be tuned or disabled via the `BEACON_FLIGHT_SQL_*` [settings](/docs/2.0.0-rc1/data-lake/configuration#arrow-flight-sql).
:::

::: warning Secure your instance
The `BEACON_ADMIN_*` credentials gate the admin UI and all write/management operations, **change them from the defaults** before exposing Beacon. To restrict who can read data, enable [access control](/docs/2.0.0-rc1/security/access-control) (`BEACON_AUTH_ENFORCE=true`).
:::
...
| | |
| - | - |
| **Explore in the browser** | [Admin Web UI](/docs/2.0.0-rc1/connect/web-admin-ui) |
| **Connect a client** | [JetBrains DataGrip](/docs/2.0.0-rc1/connect/jetbrains-datagrip) · [Python ADBC](/docs/2.0.0-rc1/connect/python-adbc) · [TypeScript SDK](/docs/2.0.0-rc1/connect/beacon-typescript-sdk) |
| **Register datasets as SQL tables** | [External Tables](/docs/2.0.0-rc1/beacondb/data-sources/external-tables) · [Views](/docs/2.0.0-rc1/data-lake/view) |
| **Write queries** | [SQL Guide](/docs/2.0.0-rc1/beacondb/sql/) |
| **Secure access** | [Authentication & Access Control](/docs/2.0.0-rc1/security/access-control) |
| **Tune performance** | [Performance Tuning](/docs/2.0.0-rc1/data-lake/performance-tuning) |
