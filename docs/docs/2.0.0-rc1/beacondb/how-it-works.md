...
[external table](/docs/2.0.0-rc1/beacondb/data-sources/external-tables) and queried by name.
...
- **Predicate pushdown**: filters become file, row-group, and chunk pruning. A time-range filter on a
  [Zarr](/docs/2.0.0-rc1/beacondb/data-sources/formats/zarr) store fetches only the intersecting chunks;
  on [Atlas](/docs/2.0.0-rc1/beacondb/data-sources/formats/atlas) it can drop whole datasets using stored
  statistics before reading any array data.
- **Federated pushdown**: for [SQL databases](/docs/2.0.0-rc1/beacondb/data-sources/sql-databases) and
  [remote Beacons](/docs/2.0.0-rc1/beacondb/data-sources/remote-tables), filters, projections, limits and
...
[File Formats](/docs/2.0.0-rc1/beacondb/data-sources/formats/).

## Storage: what BeaconDB owns

Most data stays where it is. What BeaconDB *owns* lives in a single
[`beacon.db` file](/docs/2.0.0-rc1/beacondb/data-sources/internal-format): its catalog (table, view, and
...
on for served boundaries; see [access control](/docs/2.0.0-rc1/security/access-control).

## The same engine, served

[Beacon Data Lake](/docs/2.0.0-rc1/getting-started) is this engine with a service layer around it: an
