...
This chapter covers querying remote tables. For the DDL that creates them (`LOCATION`, `OPTIONS`, TLS), schema handling, and the full list of limitations, see the [Remote Tables (Federation) setup chapter](/docs/2.0.0-rc1/beacondb/data-sources/remote-tables).
...
:::tip
Keep remote-pushed predicates to standard SQL comparisons. A filter or projection that uses a Beacon UDF (e.g. a geospatial `st_*` function) is only pushed down if the remote instance has the same function; otherwise Beacon falls back to local execution. See the [setup chapter's limitations](/docs/2.0.0-rc1/beacondb/data-sources/remote-tables#limitations).
:::
...
- **Credentials**: `'username'`/`'password'` (HTTP Basic) or a bearer `'token'`, or the name of a
  [`TYPE BEACON` secret](/docs/2.0.0-rc1/beacondb/sql/secrets) via `'secret'`, exactly one. Omit for anonymous. The
  remote enforces its own [RBAC](/docs/2.0.0-rc1/security/access-control) against whoever you authenticate
  as.
...
[Remote catalogs](/docs/2.0.0-rc1/beacondb/python/remote-catalogs).
