...
Views work over [table functions](/docs/2.0.0-rc1/beacondb/sql/table-functions) as well as external tables, useful when you want to expose a specific file set as a named table:
...
Use a view to expose multiple datasets with different schemas as a single unified table. See [UNION ALL BY NAME](/docs/2.0.0-rc1/beacondb/sql/union-by-name) for how column matching and type widening work.
