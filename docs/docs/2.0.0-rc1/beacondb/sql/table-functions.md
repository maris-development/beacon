...
Table functions let you query files directly in a `FROM` clause without creating a persistent [External Table](/docs/2.0.0-rc1/beacondb/data-sources/external-tables) first. They are useful for ad-hoc exploration or when you want to embed the file path logic inside a [View](/docs/2.0.0-rc1/data-lake/view).
...
Reads [Atlas](/docs/2.0.0-rc1/beacondb/data-sources/formats/atlas) array stores matching one or more glob patterns. Each path must point at an `atlas.json` marker file, an exact path or a glob such as `**/atlas.json`.
...
Reads a [Delta Lake](/docs/2.0.0-rc1/beacondb/data-sources/formats/delta-lake) table. Unlike the other functions, `location` is a **single path to the Delta table directory** (the folder containing `_delta_log/`), not a glob or a list. The schema is read from the transaction log.
...
To register a Delta table persistently (and to `INSERT INTO` it), use [`CREATE EXTERNAL TABLE … STORED AS DELTA`](/docs/2.0.0-rc1/beacondb/data-sources/formats/delta-lake).
