...
scoped** secret, so `read_parquet('s3://…')` and [`ATTACH`](/docs/2.0.0-rc1/beacondb/sql/remote-tables) resolve
...
[`beacondb.connect`](/docs/2.0.0-rc1/beacondb/python/secrets)) and a file-backed database, Beacon **refuses to
write a plaintext credential to disk**. Only the name/type/scope are stored in the clear; the values
are encrypted.

## Remote-Beacon secrets

A `TYPE BEACON` secret stores the credentials for [`ATTACH`](/docs/2.0.0-rc1/beacondb/sql/remote-tables):
