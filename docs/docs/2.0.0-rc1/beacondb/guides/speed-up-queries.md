...
- **Partition** Hive-style (`year=2024/month=01/`) and declare the partition columns with
  ...
  [`CREATE EXTERNAL TABLE`](/docs/2.0.0-rc1/beacondb/sql/create-table).
...
- **Choose a cloud-optimized format** when data lives on object storage. See
  [Query Data on S3](/docs/2.0.0-rc1/beacondb/guides/query-s3).

## 5. Consolidate large array collections with Atlas

For repeated queries over big NetCDF or Zarr collections, converting to
[Atlas](/docs/2.0.0-rc1/beacondb/data-sources/formats/atlas) is usually the single largest win. Atlas
...
A [materialized view](/docs/2.0.0-rc1/beacondb/sql/create-materialized-view) caches the result and is
refreshed on demand:
...
A [managed table](/docs/2.0.0-rc1/beacondb/data-sources/internal-format) is the right choice when you
...
See [Export Query Results](/docs/2.0.0-rc1/beacondb/guides/export-results).
