...
This is built on [`datafusion-table-providers`](https://github.com/datafusion-contrib/datafusion-table-providers) and DataFusion's federation layer, the same pushdown mechanism used by [remote tables](/docs/2.0.0-rc1/beacondb/data-sources/remote-tables).

:::tip External vs managed vs remote vs database
- An [**external table**](/docs/2.0.0-rc1/beacondb/data-sources/external-tables) reads files in Beacon's own storage in place.
- A [**managed table**](/docs/2.0.0-rc1/beacondb/sql/managed-tables) is owned by Beacon and mutable with `INSERT` / `UPDATE` / `DELETE`.
- A [**remote table**](/docs/2.0.0-rc1/beacondb/data-sources/remote-tables) is a federated pointer at a table on another Beacon instance.
