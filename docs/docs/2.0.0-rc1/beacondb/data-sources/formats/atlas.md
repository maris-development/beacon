...
For a stable, reusable table name, register the store as an [External Table](/docs/2.0.0-rc1/beacondb/data-sources/external-tables#atlas). Like Zarr, point the `LOCATION` at the `atlas.json` marker (or a glob over several markers):
...
:::tip
Heavy, repeated aggregations over an Atlas collection, or any other table, can be cached with a [materialized view](/docs/2.0.0-rc1/beacondb/sql/create-materialized-view) and recomputed with `REFRESH` when the underlying data changes.
:::
