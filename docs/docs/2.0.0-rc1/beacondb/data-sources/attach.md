...
table becomes queryable as `name.schema.table`, without registering tables one at a time. It is the catalog-level counterpart to a single [remote table](/docs/2.0.0-rc1/beacondb/data-sources/remote-tables): same
...
reach for a [remote table](/docs/2.0.0-rc1/beacondb/data-sources/remote-tables) when you only need one table under a fixed local name.
...
- **Credentials**: exactly one of `'username'`/`'password'` (HTTP Basic), a bearer `'token'`, or the
  name of a [`TYPE BEACON` secret](/docs/2.0.0-rc1/beacondb/sql/secrets) via `'secret'`. Omit them for
  anonymous access. The remote enforces its own [RBAC](/docs/2.0.0-rc1/security/access-control) against whoever
  you authenticate as.
...
`EXPLAIN` to confirm what is pushed down. See [Remote Tables](/docs/2.0.0-rc1/beacondb/data-sources/remote-tables#how-pushdown-works)
...
[Remote catalogs](/docs/2.0.0-rc1/beacondb/python/remote-catalogs) for the Python API.
