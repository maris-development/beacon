...
Other table functions: `read_arrow`, `read_csv`, `read_odv_ascii`, `read_bbf`, `read_tiff`. See [Reading Files](/docs/2.0.0-rc1/beacondb/sql/table-functions) for full signatures.

## Output formats

See [Querying, Output formats](/docs/2.0.0-rc1/api/querying/#output-formats) for the full list. Add `output` alongside `sql` in the same request body:
