...
See [Excluded variables](/docs/2.0.0-rc5/arrays-to-tables#excluded-variables).
...
the list smaller. See [Arrays to tables](/docs/2.0.0-rc5/arrays-to-tables).
...
[Supported calendars](/docs/2.0.0-rc5/cf-decoding#supported-calendars).
...
See [CF decoding](/docs/2.0.0-rc5/cf-decoding).
...
See [`read_<format>_schema`](/docs/2.0.0-rc5/sql/table-functions-utility#read-format-schema).
...
datasets store is that bucket. See [NetCDF](/docs/2.0.0-rc5/formats/netcdf).
...
[crawler](/docs/2.0.0-rc5/server/crawlers) with a schedule.
...
tree, use a [crawler](/docs/2.0.0-rc5/server/crawlers). The crawler does this work in advance.
...
See [Speed up slow queries](/docs/2.0.0-rc5/guides/speed-up-queries).
...
over Flight SQL. See [Export query results](/docs/2.0.0-rc5/guides/export-results).

A server-side query can spill to disk. The spill goes to the OS temp area. Put that on fast storage
with free space. See [Performance Tuning](/docs/2.0.0-rc5/server/performance-tuning).
...
See [Configuration](/docs/2.0.0-rc5/server/configuration).
...
[Secrets](/docs/2.0.0-rc5/sql/secrets).
...
For an authenticated connection, use [`ATTACH`](/docs/2.0.0-rc5/data-sources/attach) with a secret.
