...
It runs SQL and the [JSON query DSL](/docs/2.0.0-rc5/api/querying/json). It
...
[`beacon-clients/beacon-ts`](https://github.com/maris-development/beacon/tree/main/beacon-clients/beacon-ts).
...
Give the client admin Basic auth credentials. It can then manage the server.
...
`GET /api/admin/check`. The [REST API reference](/docs/2.0.0-rc5/api/) lists every
admin endpoint.
...
[admin path alias](/docs/2.0.0-rc5/api/#admin-path-alias) for the rules.
...
[Admin Web UI](/docs/2.0.0-rc5/connect/web-admin-ui) uses this SDK only. It is a
good example.
