...
[`@beacon/client`](/docs/2.0.0-rc5/connect/typescript) TypeScript SDK
only.
...
The UI calls the [admin path alias](/docs/2.0.0-rc5/api/#admin-path-alias) of the
...
- **Tables**: browse the registered tables, their Arrow schemas and their
  ...
  [external table](/docs/2.0.0-rc5/data-sources/external-tables) over the
...
- **Datasets**: explore the dataset files that Beacon finds. Inspect the schema of
  each file.
- **Crawlers**: list, [create, run and delete crawlers](/docs/2.0.0-rc5/server/crawlers).
- **Users and roles**: manage the
  [role-based access control](/docs/2.0.0-rc5/security/access-control). This covers
