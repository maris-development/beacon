...
[remote table](/docs/2.0.0-rc3/data-sources/remote-tables). Both use Arrow Flight SQL
...
[remote table](/docs/2.0.0-rc3/data-sources/remote-tables) when you need one table under a
fixed local name.
...
- **Credentials**: give exactly one of three options. Use `'username'` and `'password'` for HTTP
  ...
  [`TYPE BEACON` secret](/docs/2.0.0-rc3/sql/secrets). Omit all three for anonymous access.
  The remote applies its own [RBAC](/docs/2.0.0-rc3/security/access-control) to your identity.
...
[Remote Tables](/docs/2.0.0-rc3/data-sources/remote-tables#how-pushdown-works) for the
...
[Python client](/docs/2.0.0-rc3/connect/python), the [CLI](/docs/2.0.0-rc3/connect/cli) or the HTTP
API. See [Remote Tables](/docs/2.0.0-rc3/sql/remote-tables) for the table-level equivalent.
