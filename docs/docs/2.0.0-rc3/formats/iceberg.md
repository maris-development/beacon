...
- An [**external table**](/docs/2.0.0-rc3/data-sources/external-tables) reads a directory or a glob
  of files in place. It is read-only.
- A [**managed table**](/docs/2.0.0-rc3/sql/managed-tables) belongs to Beacon. Lance holds the data.
...
See the [`read_iceberg`](/docs/2.0.0-rc3/sql/table-functions#read-iceberg) table function.
...
[Configuration](/docs/2.0.0-rc3/server/configuration) for the storage setup.
...
- **Read-only.** Beacon writes no Iceberg table. `INSERT`, `UPDATE`, `DELETE`, `MERGE` and snapshot
  expiry fail. Use a [managed table](/docs/2.0.0-rc3/sql/managed-tables) to change rows. You also
