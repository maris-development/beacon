...
[Delta Lake](/docs/2.0.0-rc3/formats/delta-lake) is the other such format. ODV
also has no `CREATE EXTERNAL TABLE ... STORED AS ODV` form. Point `read_odv_ascii()` at the files.
Wrap the call in a [view](/docs/2.0.0-rc3/server/view) to get a stable name:
...
See [Data Sources](/docs/2.0.0-rc3/data-sources/) for the full read model.
