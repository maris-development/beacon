...
[`read_parquet`](/docs/2.0.0-rc3/sql/table-functions), `read_netcdf`, `read_zarr`,
...
[Supported formats](/docs/2.0.0-rc3/server/datasets).
...
- **Remote tables and catalogs**: tables on *another* Beacon. Beacon reaches them over Flight SQL and
  ...
  [`ATTACH`](/docs/2.0.0-rc3/sql/remote-tables).
...
then name it in [`ATTACH`](/docs/2.0.0-rc3/data-sources/attach). Beacon encrypts a persistent secret
into the `beacon.db` file. See [Secrets](/docs/2.0.0-rc3/sql/secrets).

Storage credentials are not secrets. A server reads one store, local or one bucket, chosen at
startup from [configuration](/docs/2.0.0-rc3/server/configuration).
...
[SQL reference](/docs/2.0.0-rc3/sql/) and the
[querying guide](/docs/2.0.0-rc3/api/querying/).
