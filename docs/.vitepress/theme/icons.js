// Lucide icons (https://lucide.dev), ISC-licensed. The inner SVG markup is
// inlined here so no icon dependency ships at runtime. Rendered by <Icon> in
// components, or wrapped with iconSvg() for VitePress feature `icon.svg`.
...
// Standalone <svg> string with Lucide's shared attributes — for use where a
// full SVG string is needed (e.g. VitePress home `features[].icon.svg`).
