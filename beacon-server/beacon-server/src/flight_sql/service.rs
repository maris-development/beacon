...
pub(crate) struct BeaconFlightSqlService {
    runtime: Arc<crate::server::Server>,
...
impl BeaconFlightSqlService {
    /// Creates a service using the server's configured anonymous-access policy.
    pub(crate) fn new(server: Arc<crate::server::Server>) -> anyhow::Result<Self> {
        let allow_anonymous = server.config().flight_sql.allow_anonymous;
        Self::new_with_options(server, allow_anonymous)
    }
    ...
    pub(super) fn new_with_options(
        ...
        let flight_sql = server.config().flight_sql.clone();
...
/// the server's configured policy, so tests can exercise both postures.
///
...
pub fn flight_service(
    ...
    let service = BeaconFlightSqlService::new_with_options(server, allow_anonymous)?;
    Ok(FlightServiceServer::new(service))
}

/// Starts the Flight SQL gRPC server on the configured host and port.
pub async fn serve(server: Arc<crate::server::Server>) -> anyhow::Result<()> {
    let flight_sql = server.config().flight_sql.clone();
    let service = BeaconFlightSqlService::new(server)?;
