...
impl FlightSqlMetadata {
    /// Creates metadata builders bound to the shared server.
    pub(super) fn new(server: Arc<crate::server::Server>) -> anyhow::Result<Self> {
        Ok(Self {
            server,
    ...
    pub(super) async fn build_catalogs_batch(
        ...
        let mut catalogs: Vec<String> = catalog::list_qualified_tables(&self.server, identity)
            .await
    ...
    pub(super) async fn build_schemas_batch(
        ...
        let mut schemas: Vec<(String, String)> =
            catalog::list_qualified_tables(&self.server, identity)
                .await
                .map_err(to_internal_status)?
                .into_iter()
    ...
    pub(super) async fn build_tables_batch(
        ...
        let tables = catalog::list_qualified_tables(&self.server, identity.clone())
            .await
            .map_err(to_internal_status)?;
        for (catalog_name, schema_name, table_name) in tables {
            ...
            let table_schema = catalog::table_arrow_schema(&self.server, table_ref, identity.clone())
                .await
