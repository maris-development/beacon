...
lazy_static! {
    /// Fetched from: https://vocab.nerc.ac.uk/collection/P25/current/SALIN/
...
pub fn map_seadatanet_instrument_l05_salinity() -> ScalarUDF {
    create_udf(
        "map_seadatanet_instrument_l05_salinity",
        vec![datafusion::arrow::datatypes::DataType::Utf8],
        datafusion::arrow::datatypes::DataType::Utf8,
        datafusion::logical_expr::Volatility::Immutable,
        Arc::new(map_seadatanet_instrument_l05_salinity_impl),
    )
}

fn map_seadatanet_instrument_l05_salinity_impl(
...
mod tests {
    ...
    fn test_fallback_first_value() {
        // None of the values are salinity L05 codes, so the first value is used.
    ...
    fn test_impl_scalar() {
        ...
        let result = map_seadatanet_instrument_l05_salinity_impl(&[input]).unwrap();

        match result {
            ColumnarValue::Scalar(ScalarValue::Utf8(value)) => {
                // 134 is not a salinity L05 code, so the first matching code (350) is used.
                assert_eq!(value, Some("SDN:L05::350".to_string()));
            }
    ...
    fn test_impl_array() {
        let input = ColumnarValue::Array(Arc::new(StringArray::from(vec![
            Some("salinity sensor(350)|CTD(130)"),
            Some("no codes here"),
            None,
        ])));
        ...
        match result {
            ColumnarValue::Array(array) => {
                ...
                assert_eq!(array.value(0), "SDN:L05::350");
