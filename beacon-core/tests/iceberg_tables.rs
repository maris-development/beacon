...
async fn iceberg_create_insert_select_ctas_drop() {
    ...
    let _ = runtime
        .run_query(
            beacon_core::query::Query::sql(format!("DROP TABLE IF EXISTS {copy}")),
            beacon_core::AuthIdentity::system(),
        )
        .await;
    ...
    run(
        &runtime,
        &format!("CREATE TABLE {copy} AS SELECT * FROM {table}"),
    )
    .await;
    ...
    run(
        &runtime,
        &format!("UPDATE {table} SET name = 'Z' WHERE id = 1"),
    )
    .await;
    let updated =
    ...
    let distinct_names = scalar_count(
        &run(
            &runtime,
    ...
    let any_name =
        scalar_string(&run(&runtime, &format!("SELECT name FROM {table} LIMIT 1")).await);
    ...
    let non_null_age =
        scalar_count(&run(&runtime, &format!("SELECT count(age) FROM {copy}")).await);
    assert_eq!(
        non_null_age, 0,
        "existing rows must read NULL for a new column"
    );
    ...
    run(
        &runtime,
        &format!("ALTER TABLE {copy} ALTER COLUMN age TYPE BIGINT"),
    )
    .await;
    ...
    run(
        &runtime,
        &format!("ALTER TABLE {copy} RENAME COLUMN name TO label"),
    )
    .await;
    let label =
        scalar_string(&run(&runtime, &format!("SELECT label FROM {copy} WHERE id = 1")).await);
    ...
    let narrow = runtime
        .run_query(
            beacon_core::query::Query::sql(format!("ALTER TABLE {copy} ALTER COLUMN age TYPE INT")),
            beacon_core::AuthIdentity::system(),
        )
        .await;
    ...
    run(
        &runtime,
        &format!("UPDATE {table} SET name = 'a' WHERE id = 1"),
    )
    .await;
    run(
        &runtime,
        &format!("UPDATE {table} SET name = 'b' WHERE id = 2"),
    )
    .await;
    ...
    run(
        &runtime,
        &format!("CREATE VIEW {view_name} AS SELECT 1 AS id"),
    )
    .await;
    let delete_view = runtime
        .run_query(
            beacon_core::query::Query::sql(format!("DELETE FROM {view_name} WHERE id = 1")),
            beacon_core::AuthIdentity::system(),
        )
        .await;
    assert!(
    ...
    let update_view = runtime
        .run_query(
            beacon_core::query::Query::sql(format!("UPDATE {view_name} SET id = 2 WHERE id = 1")),
            beacon_core::AuthIdentity::system(),
        )
        .await;
    assert!(
    ...
    let alter_view = runtime
        .run_query(
            beacon_core::query::Query::sql(format!("ALTER TABLE {view_name} ADD COLUMN x INT")),
            beacon_core::AuthIdentity::system(),
        )
        .await;
    assert!(
