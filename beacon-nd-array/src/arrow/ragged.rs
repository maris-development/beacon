//! Streaming a [`RaggedDataset`] (CF contiguous ragged-array convention) as
//! Arrow [`RecordBatch`]es.
//!
//! Casts are grouped into batches up to `batch_size` observation rows; each
//! planned batch range is an independent read unit (see [`read_ragged_unit`])
//! so it can be driven sequentially ([`ragged_dataset_as_record_batch_stream`])
//! or concurrently (by the parallel producer in [`super::parallel`]).
...
pub(crate) fn ragged_dataset_as_record_batch_stream(
    ...
    futures::stream::once(async move {
        plan_ragged_stream(ragged, batch_size, predicate, &metrics)
            .await
            .map(|plan| (plan, metrics))
    })
    .map(|init| match init {
        Ok((plan, metrics)) => {
...
/// Everything needed to read the batches of a ragged dataset, computed once
/// up front (offsets, instance-pushdown mask, and the planned batch ranges).
pub(crate) struct RaggedStreamPlan {
    pub(crate) ragged: RaggedDataset,
    pub(crate) schema: Arc<Schema>,
    pub(crate) obs_dims: HashSet<String>,
    pub(crate) offsets: HashMap<String, Vec<usize>>,
    pub(crate) passing_indices: Vec<usize>,
    pub(crate) ranges: Vec<Range<usize>>,
}

/// Compute the read plan for a ragged dataset: resolve cumulative offsets,
/// apply instance-variable pushdown, account for pruned casts in `metrics`,
/// and plan the per-batch cast ranges.
pub(crate) async fn plan_ragged_stream(
    ragged: RaggedDataset,
    batch_size: usize,
    predicate: Option<PushdownFilter>,
    metrics: &Option<DatasetReadMetrics>,
) -> anyhow::Result<RaggedStreamPlan> {
    let schema = ragged_record_batch_schema(&ragged);
    let obs_dims: HashSet<String> = ragged.observation_dimensions().map(String::from).collect();
...
/// Read one planned batch range of a ragged dataset into a [`RecordBatch`],
/// updating output `metrics`. Each call is independent of the others, so this
/// can be driven sequentially or concurrently.
pub(crate) async fn read_ragged_unit(
    ragged: &RaggedDataset,
    schema: &Arc<Schema>,
    obs_dims: &HashSet<String>,
    ...
    let batch_indices: Vec<usize> = passing_indices[range.start..range.end].to_vec();

    // Read each cast and merge (can't use get_casts_range since indices may not be contiguous).
    let cast_data = if let (Some(&first), Some(&last)) = (batch_indices.first(), batch_indices.last())
    {
    ...
    let batch =
        ragged_batch_to_record_batch(&cast_data, schema, obs_dims, offsets, &bounding_range).await?;
...
mod tests {
    ...
    use crate::{
        NdArray, NdArrayD,
        dataset::{Dataset, any::AnyDataset},
    };
    ...
    /// (instance dim `casts`=3, obs dim `z_obs`=6 with row sizes [2, 1, 3]).
