...
//! server on top of a shared [`DataLake`](crate::datalake::DataLake) so both
...
fn main() -> anyhow::Result<()> {
    ...
    // a process-global.
    let config = Arc::new(beacon_datalake_config::Config::load().context("failed to load configuration")?);
...
async fn async_main(config: Arc<beacon_datalake_config::Config>) -> anyhow::Result<()> {
    setup_tracing();
    install_panic_hook();
    ...
    if config.flight_sql.enable {
        tokio::try_join!(http_server, flight_sql::serve(lake.clone()))?;
    } else {
        http_server.await?;
    }
