...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 404, description = "Table not found, or it has no persisted definition"),
    ),
    security(("basic-auth" = []))
)]
pub(crate) async fn list_table_config(
    ...
    match state.runtime().table_config(&query.table_name).await {
        Some(config) => Ok(Json(config)),
        None => {
            tracing::error!(table_name = %query.table_name, "table config not found");
            Err((
