...
impl FileFormat for GeoParquetFormat {
    ...
    async fn infer_schema(
        ...
        // Implement schema inference logic for GeoParquet format
    ...
    async fn infer_stats(
        ...
        // Implement statistics inference logic for GeoParquet format
        // This is a placeholder implementation and should be replaced with actual logic
    ...
    async fn create_physical_plan(
        ...
        // Implement physical plan creation logic for GeoParquet format
        // This is a placeholder implementation and should be replaced with actual logic
        Err(datafusion::error::DataFusionError::NotImplemented(
            "GeoParquet physical plan creation not implemented".to_string(),
        ))
    }
...
impl GeoMapper {
    pub fn new(input_schema: &Schema, geo_options: &GeoParquetOptions) -> Self {
        ...
        Self {
            longitude_idx,
            latitude_idx,
            geometry_idx: output_schema.index_of("geometry").unwrap(),
            output_schema,
        }
    }
...
impl GeoParquetSink {
    ...
    pub fn conf(&self) -> &FileSinkConfig {
    ...
    fn create_writer(&self, output_path: &str) -> GeoParquetWriter<BufWriter<std::fs::File>> {
        let file = std::fs::File::create(output_path)
            .map_err(|e| datafusion::error::DataFusionError::IoError(e))
            .unwrap();

        let buf_writer = std::io::BufWriter::new(file);

        let mut options = GeoParquetWriterOptions::default();
        options.encoding = geoarrow::io::parquet::GeoParquetWriterEncoding::Native;
...
impl DataSink for GeoParquetSink {
    ...
    async fn write_all(
        ...
        let output_path = format!(
            ...
            self.file_sink_config.table_paths[0].prefix().to_string()
        );
