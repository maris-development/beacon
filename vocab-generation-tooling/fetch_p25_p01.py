...
OUTPUT_FILE = "p25_to_p01_codes.json"
...
def extract_code(uri: str) -> str:
    """
    Extract the concept code from NVS URI.
    Example:
    http://.../P25/current/CDIAM/ -> CDIAM
    """
...
def extract_narrower_p01_codes(concept_json: dict) -> List[str]:
    codes = []

    narrower = concept_json.get("skos:narrower")
    if not narrower:
        return []
...
def main():
    ...
    for i, p25_uri in enumerate(p25_ids, start=1):
        ...
        p01_codes = extract_narrower_p01_codes(concept_json)
        mapping[p25_code] = p01_codes
