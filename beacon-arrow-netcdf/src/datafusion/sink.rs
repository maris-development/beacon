...
use crate::datafusion::execution::unique_values::{
...
impl DataSink for NetCDFNdSink {
    ...
    async fn write_all(
        ...
        let mut rows_written: u64 = 0;
        let mut nc_file = crate::netcdf::create(output_path).map_err(|e| {
            datafusion::error::DataFusionError::Execution(format!(
...
mod tests {
    use super::*;
    use crate::datafusion::execution::unique_values::UniqueColumnValues;
    use arrow::{
    ...
    fn create_test_file(
        dims: &[(&str, usize)],
    ) -> (tempfile::TempDir, crate::netcdf::FileMut) {
        let temp_dir = tempdir().expect("create tempdir");
        let file_path = temp_dir.path().join("slab.nc");
        let mut file = crate::netcdf::create(&file_path).expect("create netcdf file");
