...
# what actually runs on a `beacondb-v*` tag. This extracts it verbatim so that
# scripts/build-wheel-docker.sh and .github/workflows/wheel-toolchain.yml exercise exactly
# what the release build does, rather than a copy that drifts out of sync with it.
#
# POSIX sh + awk: this also runs inside the Alpine musllinux container, which has no bash.
set -eu
