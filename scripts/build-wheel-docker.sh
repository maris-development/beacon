...
# Match the feature set the release workflow uses, by reading it out of the workflow rather than
# repeating it. The set has grown once per native library that had to move inside the wheel
# (netCDF/HDF5, then OpenSSL, then PROJ), and a copy here would quietly test a configuration the
# release does not build.
if [[ -z "$features" ]]; then
    features=$(sed -n 's/^[[:space:]]*--features[[:space:]]*\([^[:space:]]*\).*/\1/p' "$workflow" | head -n1)
    if [[ -z "$features" ]]; then
        echo "error: no --features line found in $workflow" >&2
        exit 1
    fi
fi

# The `channel` from rust-toolchain.toml, matching what a dev machine uses. maturin-action
# resolves its own toolchain in CI, so this only has to be able to build the workspace.
rust_version=$(sed -n 's/^[[:space:]]*channel[[:space:]]*=[[:space:]]*"\([^"]*\)".*/\1/p' \
    "$repo_root/rust-toolchain.toml" 2>/dev/null | head -n1)
