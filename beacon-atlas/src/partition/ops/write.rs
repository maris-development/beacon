...
impl<S: object_store::ObjectStore + Clone> PartitionWriter<S> {
    ...
    pub(crate) async fn write_dataset<C: Stream<Item = Column>>(
        &mut self,
        name: &str,
        columns: C,
    ) -> anyhow::Result<()> {
    ...
    pub(crate) async fn finish(mut self) -> anyhow::Result<Partition> {
...
mod tests {
    ...
    use crate::{
        column::Column,
        partition::{load_partition, ops::read::ReaderBuilder},
    };
    ...
    async fn finish_writes_loadable_partition_metadata_and_entries() -> anyhow::Result<()> {
        ...
        let mut arrays = ReaderBuilder::new(store, loaded)
            .with_dataset_index(0)
            .execute()
            .await?;
