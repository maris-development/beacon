...
impl RegistrySnapshot {
    ...
    /// bytes; it is divided by count instead, which is what the listing path
    ...
    pub fn shard_prefix(&self, prefix: &str, parts: usize) -> Result<PrefixShards> {
        ...
        let share = if by_bytes {
            bytes.div_ceil(parts as u64)
        } else {
            files.div_ceil(parts as u64)
        };
...
mod tests {
    ...
    /// Shards balance on bytes, not on count: one large file is a partition of
    /// its own rather than dragging its neighbours along.
    #[test]
    fn shards_balance_on_bytes() {
        let (registry, _dir) = registry();
        registry
            .intern_files(&[
                observed("obs/a", 1000),
