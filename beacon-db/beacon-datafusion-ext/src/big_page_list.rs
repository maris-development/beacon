...
mod tests {
    ...
    impl PaginatedListStore for FakeStore {
        async fn list_paginated(
            ...
            let objects: Vec<ObjectMeta> =
                under[start..end].iter().map(|p| Self::meta(p)).collect();
        ...
        async fn list_with_delimiter(&self, prefix: Option<&Path>) -> OsResult<ListResult> {
            ...
            for path in &self.paths {
                ...
                let rest = if base.is_empty() {
                    Some(full)
                } else if full.starts_with(&base)
                    && full.as_bytes().get(base.len()) == Some(&b'/')
                {
    ...
    async fn a_sharded_walk_pages_every_shard() {
        ...
        let sizes = store.page_sizes();
        assert!(sizes.len() >= 4, "one page per leaf shard at least, got {sizes:?}");
